...
class EmailNotifier < BuilderPlugin
  ...
  def build_finished(build)
    ...
    email :build_report, build, "#{build.project.display_name} build #{build.abbreviated_label} failed",
          "The build failed."
  end

  def build_fixed(build, previous_build)
    return if @emails.empty?
    email :build_report, build, "#{build.project.display_name} build #{build.abbreviated_label} fixed",
