...
class Project
  ...
  attr_accessor :source_control, :scheduler, :use_bundler, :gemfile, :bundler_args, :display_name

  alias_method :id, :name

  def display_name; @display_name ||= self.name; end
  ...
  def load_display_name
    if File.exist?(filename = Configuration.projects_root.join(self.name, 'display_name.txt'))
      self.display_name = IO.read(filename).to_s.gsub("\n", "").strip
    end
  end
